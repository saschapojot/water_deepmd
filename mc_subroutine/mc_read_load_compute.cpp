...
void mc_computation::init_coord_and_box()
{
    ...
    else
    {
        ...
        std::memcpy(box_init.get(),
                    box_data_all_ptr.get() + (sweep_to_write - 1) * box_component_num,
...
void mc_computation::execute_mc(const std::shared_ptr<double[]>& coord_init,
                                const std::shared_ptr<double[]>& box_init, const int& flushNum)
{
    std::memcpy(coord_1_frame_curr.data(), coord_init.get(), total_atom_xyz_components_num * sizeof(double));

    std::memcpy(box_1_frame_curr.data(), box_init.get(), box_component_num * sizeof(double));

    // std::cout<<"box_1_frame_curr:";
    // print_vec(box_1_frame_curr,box_component_num);
    double UCurr = 0;
    ...
    for (int fls = 0; fls < flushNum; fls++)
    {
        ...
        for (int swp = 0; swp < sweep_to_write * sweep_multiple; swp++)
        {
            ...
            if (swp % sweep_multiple == 0)
            {
                ...
                std::memcpy(coord_data_all_ptr.get() + swp_out * total_atom_xyz_components_num,
        ...
        std::string out_box_PickleFileName = out_box_path + "/" + fileNameMiddle + ".box.pkl";
        //save U
        this->save_array_to_pickle(U_data_all_ptr, sweep_to_write, out_U_PickleFileName);
        //save coord
        this->save_array_to_pickle(coord_data_all_ptr, sweep_to_write * total_atom_xyz_components_num,
                                   out_coord_PickleFileName);

        //save box
        this->save_array_to_pickle(box_data_all_ptr, sweep_to_write * box_component_num,
                                   out_box_PickleFileName);
...
void mc_computation::proposal_uni_coord(const std::vector<double>& coord_1_frame_curr,
                                        ...
                                        const std::vector<double>& box_1_frame_curr)
{
...
void mc_computation::proposal_uni_box(const std::vector<double>& box_1_frame_curr,
                                      std::vector<double>& box_1_frame_next, const int& ind,
                                      const std::vector<double>& coord_1_frame_curr,
                                      double& atom_x_max_val, double& atom_y_max_val, double& atom_z_max_val)
{
    ...
    if (ind == box_x_component_position)
    {
        ...
        atom_y_max_val = -1;
        atom_z_max_val = -1;
        double x_direction_new_val = this->generate_uni_open_interval(box_1_frame_curr[box_x_component_position],
    ...
    if (ind == box_y_component_position)
    {
        ...
        atom_x_max_val = -1;
        atom_z_max_val = -1;
        double y_direction_new_val = this->generate_uni_open_interval(box_1_frame_curr[box_y_component_position],
    ...
    if (ind == box_z_component_position)
    {
        ...
        atom_x_max_val = -1;
        atom_y_max_val = -1;
        double z_direction_new_val = this->generate_uni_open_interval(box_1_frame_curr[box_z_component_position],
...
double mc_computation::acceptanceRatio_uni_for_box(const std::vector<double>& box_1_frame_curr,
                                                   ...
                                                   const double& atom_x_max_val, const double& atom_y_max_val,
                                                   const double& atom_z_max_val)
{
...
double mc_computation::acceptanceRatio_uni_for_coord(const std::vector<double>& coord_1_frame_curr,
                                                     const std::vector<double>& coord_1_frame_next, const int& ind,
                                                     const double& UCurr, const double& UNext,
                                                     const double& box_x_val, const double& box_y_val,
                                                     const double& box_z_val)
{
...
void mc_computation::energy_update_box_one_component(const std::vector<double>& box_1_frame_curr,
                                                     const std::vector<double>& box_1_frame_next,
                                                     const std::vector<double>& coord_1_frame_curr, double& UCurr,
                                                     double& UNext)
{
    //energy curr
    this->dp_model.compute(UCurr, force_1_frame_curr, virial_1_frame_curr, coord_1_frame_curr, atom_type,
                           box_1_frame_curr);

    //energy next
    this->dp_model.compute(UNext, force_1_frame_next, virial_1_frame_next, coord_1_frame_curr, atom_type,
                           box_1_frame_next);
}
...
void mc_computation::energy_update_coord_one_component(const std::vector<double>& coord_1_frame_curr,
                                                       const std::vector<double>& coord_1_frame_next, double& UCurr,
                                                       double& UNext, const std::vector<double>& box_1_frame_curr)
{
    //energy curr
    this->dp_model.compute(UCurr, force_1_frame_curr, virial_1_frame_curr, coord_1_frame_curr, atom_type,
                           box_1_frame_curr);
    ...
    //energy next
    this->dp_model.compute(UNext, force_1_frame_next, virial_1_frame_next, coord_1_frame_next, atom_type,
                           box_1_frame_curr);
}
                                          ...
                                          std::vector<double>& box_1_frame_next)
{
    ...
    for (int i = 0; i < total_atom_xyz_components_num; i++)
    {
        ...
        //std::cout<<"u="<<u<<", r_coord="<<r_coord<<std::endl;
        if (u <= r_coord)
        {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // std::cout<<"ind_box="<<ind_box<<std::endl;
        ...
        double u = distUnif01(e2);
        // std::cout<<"u="<<u<<", r_box="<<r_box<<std::endl;
        if (u <= r_box)
        {
            UCurr = UNext;
